...
export default function PalletsV47({data,onChange,audit,currentUser}){
 ...
 const registrar=()=>{
  ...
  const origem=form.tipo==="COMPRA"?"FORA DO PATRIMÔNIO":form.origem;
  const destino=form.tipo==="VENDA"?"BAIXA DEFINITIVA":form.destino;
  ...
  const movimento={id:id(),...form,origem,destino,quantidade:q,custoUnitario:Number(form.custoUnitario||0),valorTotal:q*Number(form.custoUnitario||0),afetaPatrimonio:["COMPRA","VENDA","PERDA / AVARIA","AJUSTE DE ENTRADA","AJUSTE DE SAÍDA"].includes(form.tipo),dataHora:now(),usuario:currentUser?.nome||""};
