...
export default function FinanceiroHub({data,onChange,currentUser}){
 ...
 const posicaoEstoque=useMemo(()=>{const movimentos=data.estoqueMov||[];return (data.produtos||[]).map(p=>{const lista=movimentos.filter(m=>m.produtoId===p.id).slice().sort((a,b)=>String(a.dataHora||a.data||"").localeCompare(String(b.dataHora||b.data||"")));let inicial=0,entradas=0,saidas=0;for(const m of lista){const dataMov=String(m.dataHora||m.data||"").slice(0,10);if(!dataMov||dataMov>dia)continue;const tipo=String(m.tipo||"");const q=Number(m.quantidade||0);const delta=tipo.startsWith("ENTRADA")?q:tipo.startsWith("SAÍDA")?-q:Number(m.ajuste??q);if(dataMov<dia)inicial+=delta;else if(delta>=0)entradas+=delta;else saidas-=delta}return {id:p.id,produto:p.nome||p.descricao||"PRODUTO",marca:p.marca||"",inicial,entradas,saidas,final:inicial+entradas-saidas}}).filter(p=>p.inicial||p.entradas||p.saidas||p.final)},[data.estoqueMov,data.produtos,dia]);
 ...
 return <main className="main" style={{maxWidth:1300,margin:"auto",padding:20}}>
   ...
   {aba==="ESTOQUE"&&<section className="card"><h2>ESTOQUE POR PRODUTO</h2><p>POSIÇÃO REGISTRADA ATÉ A DATA SELECIONADA. O SALDO É CALCULADO PELAS MOVIMENTAÇÕES DE ENTRADA, SAÍDA E AJUSTE.</p><label>DATA <input type="date" value={dia} onChange={e=>{setDia(e.target.value);setCaixaId("")}}/></label>{!posicaoEstoque.length?<p>NENHUMA MOVIMENTAÇÃO DE ESTOQUE REGISTRADA ATÉ ESTA DATA.</p>:<div style={{overflowX:"auto",marginTop:16}}><table style={{width:"100%"}}><thead><tr><th>PRODUTO</th><th>MARCA</th><th>SALDO INICIAL</th><th>ENTRADAS DO DIA</th><th>SAÍDAS DO DIA</th><th>SALDO FINAL</th></tr></thead><tbody>{posicaoEstoque.map(p=><tr key={p.id}><td>{p.produto}</td><td>{p.marca}</td><td>{p.inicial}</td><td>{p.entradas}</td><td>{p.saidas}</td><td><b>{p.final}</b></td></tr>)}</tbody></table></div>}<p>O FECHAMENTO DEFINITIVO DO CAIXA FICA NA ABA FECHAMENTOS.</p></section>}
